...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['delete'])) {
    ...
    // Verifica se o email ou CPF do professor já existe
    ...
    if ($stmt_check->num_rows > 0) {
        echo "<div class='alert alert-danger'>Erro: Este email ou CPF já está cadastrado!</div>";
    } else {
        // Inserindo no banco de dados
        $sql = "INSERT INTO professores (nome, email, cpf, senha) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssss", $nome, $email, $cpf_limpo, $senha);
